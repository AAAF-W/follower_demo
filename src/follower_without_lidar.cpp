...
void MoveCallback(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    ...
    // ROS_WARN("[follower_without_lidar] = ( %.2f , %.2f )  %.2f",target_pose.pose.position.x ,target_pose.pose.position.y,fYaw);
}
...
{
    ros::init(argc, argv, "follower_without_lidar"); 
    ...
    if(n_param.getParam("robot_id", Robot_ID))
    {
        ...
        ROS_WARN("[follower_without_lidar] with (%s) start!",name_space.c_str());
    }
    else
    {
        InitHelper("LocalMap");
    ROS_WARN("[follower_without_lidar] start!");
    }
    ...
    while(ros::ok())
    {
        ...
        //ROS_WARN("[follower_without_lidar] = ( %.2f , %.2f ) ",target_pose.pose.position.x ,target_pose.pose.position.y);
