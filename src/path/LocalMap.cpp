...
static double x_sin[360];
static double y_cos[360];
static int pnt_x[360];
static int pnt_y[360];
...
void InitHelper(string inTitle)
{
	...
	for (int i = 0; i < 360; i++)
	{
		x_sin[i] = 20*sin(M_PI*0.0 - kStep*i);
		y_cos[i] = -20*cos(M_PI*0.0 - kStep*i);
	}
...
void SetRanges(float* inData)
{
	...
	// 队友也当作障碍物
	SetTMBlock(team_mate_x[0] ,team_mate_y[0]);
	SetTMBlock(team_mate_x[1] ,team_mate_y[1]);
}
		...
		for (int y = inY - nDepthR; y <= inY + nDepthR; y++)
		{
			for (int x = inX - nDepthR; x <= inX + nDepthR; x++)
			{
				if (x >= 0 && x < MAP_WIDTH && y >= 0 && y < MAP_HEIGHT)
				{
					...
					int nFixedVal = *pTmpMark;// + inIndex ;
					if (*pMapPoint < nFixedVal)
					{
						*pMapPoint = nFixedVal;
					}
				}
